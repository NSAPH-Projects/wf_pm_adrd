...
data4casecross_zip <- read_fst('/n/dominici_nsaph_l3/Lab/projects/wfpm_adrd_hosp/data/data_process/data4casecross/data_w_all_vars/data4casecross_zip_2006cohort.fst')
...
# for simulations, distribution of cardiorespiratory outcomes and exposure
summary(data4casecross_zip$count_resp_prmy_out)
summary(data4casecross_zip$wfpm_lag00)
...
analyze_dlnm_wfpm <- function(ExpTerm, CaseType, Sensitivity,  
  ...
  if(str_detect(ERConstraint, 'evenknots') & str_detect(LRConstraint, 'evenknots')){
    ...
    if(str_detect(Sensitivity, regex('(?i)Main'))){
      est.table %>%
        write.csv(paste0(output_path, '/Estimates/Main/',
                         'EstInd_', ModelName, '_2006cohort.csv'))
    }
    ...
    if(str_detect(Sensitivity, regex('(?i)Main'))){
      est.table %>%
        write.csv(paste0(output_path, '/Estimates/Main/',
                         'EstCumul_', ModelName, '_2006cohort.csv'))
    }
...
analyze_dlnm_wfpm(ExpTerm = 'zip_daily_wfpm', dta = data4casecross_zip,
                  CaseType = "count_circ_prmy_out", Sensitivity = "Main",
...
# respiratory
analyze_dlnm_wfpm(ExpTerm = 'zip_daily_wfpm', dta = data4casecross_zip,
                  CaseType = "count_resp_prmy_out", Sensitivity = "Main",
...
# anxiety
analyze_dlnm_wfpm(ExpTerm = 'zip_daily_wfpm', dta = data4casecross_zip,
                  CaseType = "count_anxty_prmy_out", Sensitivity = "Main",
...
# depression
analyze_dlnm_wfpm(ExpTerm = 'zip_daily_wfpm', dta = data4casecross_zip,
                  CaseType = "count_dep_prmy_out", Sensitivity = "Main",  
                  ERConstraint = "lin", LRConstraint = "5dfevenknots",
                  lag_number = 7)
