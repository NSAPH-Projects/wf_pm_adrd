...
# wf pm - hospitalizations by wfpm pattern cluster
# Vivian Do
# Updated 12/1/23
...
analyze_dlnm_wfpm_cluster <- function(ExpTerm, CaseType, Sensitivity, cluster_name,  
                              ERConstraint, LRConstraint, dta, lag_number){
  # 
  # first_10_unique <- head(distinct(data4casecross_zip, id), 1000)
  # data4casecross_ind_smaller <- data4casecross_zip %>%
  #   filter(id %in% first_10_unique$id)
  # # 
  # ExpTerm <- 'zip_daily_wfpm'; CaseType <- 'count_circ_prmy_out'; Sensitivity <- 'Main'; # Sensitivity <- 'Sens_lag14';
  # ERConstraint <- '2dfevenknots'; LRConstraint <- '4dfevenknots'; dta <- data4casecross_zip
  
  print(paste0("Outcome: ", CaseType, ", stratification var: ", cluster_name))
  
  # filter to days with cases along with their controls (bc a case cannot be a day without 0 events)
  dta <- dta %>% 
    filter(!!sym(CaseType) > 0,
           !!sym(cluster_name) == strat_unique_levels[[strat_unique_level]])
