...
analyze_dlnm_wfpm <- function(ExpTerm, CaseType, Sensitivity,  
  ...
  # ExpTerm <- 'wfpm'; CaseType <- 'count_ADRD_first_two_codes_out';
  # Sensitivity <- 'Main'; ERConstraint <- '2dfevenknots'; LRConstraint <- '4dfevenknots'; #ERConstraint <- '4dfevenknots'
...
# run main models
analyze_dlnm_wfpm('daily_wfpm', 'count_ADRD_prmy_out', 'Main', 
                  '1dfevenknots', '4dfevenknots', 
                  'SaveModel', samp_data_25pct)

analyze_dlnm_wfpm('daily_wfpm', 'count_ADRD_prmy_out', 'Main', 
                  '2dfevenknots', '4dfevenknots', 
                  'SaveModel', samp_data_25pct)

analyze_dlnm_wfpm('daily_wfpm', 'count_ADRD_prmy_out', 'Main', 
                  '3dfevenknots', '4dfevenknots', 
                  'SaveModel', samp_data_25pct)
...
Daily_wfpm.max <- max(obsHourlyNO2, na.rm = TRUE)
...
expContrasts <- data.frame(  
  Counterfactual_wfpm = c(seq(HourlyNO2.min, Daily_wfpm.max, length.out = 100), 
