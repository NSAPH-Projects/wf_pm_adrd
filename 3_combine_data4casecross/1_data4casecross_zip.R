...
# Read in data ------------------------------------------------------------
#* this zip_count_hosp_outcomes data contains all years
#* only need to load this once while loop goes through all years
zip_count_hosp_outcomes <- read_rds(paste0(data_path, "/data_process/hosp_dta/hosp_after_first_adrd_zip.rds")) %>% 
...
keep_data_w_all_wfpm <- function(data4casecross_year){
  ...
  # missingness could be an issue with zcta-zip linkage that we need for urbanicity/poverty vars
...
# overlap possible, e.g., a zip day could have cardioresp and adrd
data4casecross_zip_final <- bind_rows(data4casecross_zip) %>% 
  ...
  mutate(pct_pov_final = ifelse(year(DayDateTime) <= 2011, pct_pov_0711, pct_pov_1216),
         ...
         hosp_of_interest = if_any(contains("count"), ~. > 0, na.rm = TRUE)) %>% # keep only hospitalizations of interest bc a person could be hospitalized for many other reasons
  filter(hosp_of_interest == TRUE) %>% 
  select(-contains(c("0711", "1216")), -hosp_of_interest)

data4casecross_zip_final <- read_fst('/n/dominici_nsaph_l3/Lab/projects/wfpm_adrd_hosp/data/data_process/data4casecross/data_w_all_vars/data4casecross_zip.fst')

nrow(data4casecross_zip_final) 
nrow(data4casecross_zip_final %>% group_by(id) %>% filter(row_number() == 1)) 
summary(data4casecross_zip_final %>% select(contains("prmy")))

# does maine exist in this dataset (whoo hoo yes)
data4casecross_zip_final_maine <- data4casecross_zip_final %>% 
  filter(nchar(as.character(zip)) == 4)

unique(data4casecross_zip_final$state)

# evaluate poverty assignment
table(data4casecross_zip_final$pct_pov_final, useNA = "always")
