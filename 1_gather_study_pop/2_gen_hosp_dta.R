...
# first adrd hospitalization occurence
n_zip <- first_adrd_hosp_beg_2000 %>% 
  group_by(zipcode_R) %>% 
  filter(row_number() == 1)
nrow(n_zip)

# remove zip codes from first hospitalization after getting eda
first_adrd_hosp_beg_2000 <- first_adrd_hosp_beg_2000 %>% 
  select(-zipcode_R)
...
years <- c(2006:2016)
hosp_after_first_adrd_ind <- data.frame()
...
for (year in years){
  ...
  study_hosp_data <- yrly_hosp_data %>%
    select(QID, ADATE, DIAG1, DIAG2, DIAG3, zipcode_R, ADM_TYPE, AGE, Sex_gp) %>%
    mutate(adm_date_hosp = as.Date(ADATE, format = "%d%b%Y")) %>%
    inner_join(.,
               first_adrd_hosp_beg_2000 %>%
  ...
  hosp_after_first_adrd_ind <- rbind(hosp_after_first_adrd_ind, study_hosp_data)
...
hosp_after_first_adrd_ind <- assign_hosp_outcome(hosp_after_first_adrd_ind) %>% 
  select(QID, adm_date_hosp, zipcode_R, age, sex_group, adm_type, ends_with("out"))
names(hosp_after_first_adrd_ind)

# join with the medicare beneficiary data (denominator file)
# add DOB to calculate age
hosp_after_first_adrd_ind <- hosp_after_first_adrd_ind %>% 
  left_join(., 
            bene_dob %>% 
              select(bene_id, dob) %>% 
              rename(QID = bene_id),
...
nrow(hosp_after_first_adrd_ind)

write_rds(hosp_after_first_adrd_ind, paste0(data_path, "data_process/hosp_dta/hosp_after_first_adrd_ind.rds")) 

# Generate case days dataset -------------------------------------------------------------------------
#* this is to more easily identify case and control days for exposure and confounders
hosp_after_first_adrd_case_days <- hosp_after_first_adrd_ind %>% 
  select(adm_date_hosp, zipcode_R) %>% 
  group_by(adm_date_hosp, zipcode_R) %>% 
  filter(row_number() == 1) %>% 
  ungroup()
write_rds(hosp_after_first_adrd_case_days, paste0(data_path, "data_process/hosp_dta/hosp_after_first_adrd_case_days.rds"))
# hosp_after_first_adrd_case_days <- readRDS(paste0(data_path, "data_process/hosp_dta/hosp_after_first_adrd_case_days.rds"))
...
# hosp_after_first_adrd_ind <- read_rds(paste0(data_path, "data_process/hosp_dta/hosp_after_first_adrd_ind.rds"))

hosp_after_first_adrd_zip <- hosp_after_first_adrd_ind %>% 
  group_by(adm_date_hosp, zipcode_R) %>% 
...
write_rds(hosp_after_first_adrd_zip, paste0(data_path, "data_process/hosp_dta/hosp_after_first_adrd_zip.rds")) 
