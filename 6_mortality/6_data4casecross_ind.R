...
dod_after_first_adrd_hosp <- read_rds("/n/dominici_nsaph_l3/Lab/projects/wfpm_adrd_hosp/data/data_process_mortality/mortality_dta/dod_after_first_adrd_hosp.rds") %>% 
  rename(CaseDateTime = bene_dod) %>% 
  mutate(new_id_data4casecross = row_number())  #because this is individual level data, we want the id to be unique to each individual
...
gen_data4casecross_year <- function(year){
  ...
  death_dta <- left_join(data_w_lagged_wfpm, data_w_lagged_temp_rh, by = c("zip", "DayDateTime")) %>% 
    left_join(., dod_after_first_adrd_hosp, by = c("zip",
...
data4casecross_ind_final_w_wfpm_cluster <- data4casecross_ind_final %>% 
...
write_fst(data4casecross_ind_final_w_wfpm_cluster, '/n/dominici_nsaph_l3/Lab/projects/wfpm_adrd_hosp/data/data_process_mortality/data4casecross/data_w_all_vars/data4casecross_ind.fst', compress = 100)
