...
# Author: Vivian
# Date: 12/13/23
# Goal: assign lagged days of wildfire pm for mortality analysis
####**********************
...
data_path_external <- paste0(here(data_path, "data_process_mortality/case_cntrl_days/"))
...
assign_laggedExp_zip <- function(dta_outcome, dta_exp, numLag){
  ...
  dta_outcome <- dta_outcome %>% 
    ...
    mutate(zip_activeLag = paste0(zip_dod, activeLag))
...
data4casecross_zip <- read_fst(paste0(data_path, "data_process_mortality/case_cntrl_days/cntrl_days_zip_2006.fst")) %>% 
  filter(zip_dod == "1001")
...
for (year in (2006:2016)){
  ...
  data4casecross_zip <- read_fst(paste0(data_path, "data_process_mortality/case_cntrl_days/cntrl_days_zip_", year, ".fst"))
  ...
  data4casecross_zip %>%
    ...
    # filter(row_number() == 1) %>%
    fst::write_fst(paste0(data_path, 'data_process_mortality/data4casecross/data_w_lagged_wfpm/',
                          'data4casecross_', ActiveYear, '.fst'))
  
}
end_time <- Sys.time()
end_time - start_time #approx 50 min

# Check --------------------------------------------------------------------
data4casecross_2006 <- read_fst(paste0(data_path, '/data_process_mortality/data4casecross/data_w_lagged_wfpm/data4casecross_2006.fst'))
test_1001 <- data4casecross_2006 %>% 
  filter(zip == "1001")
glimpse(data4casecross_2006)
