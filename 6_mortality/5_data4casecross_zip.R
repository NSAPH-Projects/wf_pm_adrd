...
# Author: Vivian 
# Date: 12/14/23
# Goal: combine exposure, outcome, confounder data for zip codes
# should also contain: urbanicity
####**********************
rm(list=ls())
source(here::here("0_set_up", "1_libraries.R"))
data_path <- '/n/dominici_nsaph_l3/Lab/projects/wfpm_adrd_hosp/data'
data_path_data4casecross <- paste0(data_path, '/data_process_mortality/data4casecross')
data_path_stratification_vars <- paste0(data_path, '/data_process/stratification_vars')

# Read in data ------------------------------------------------------------
#* this zip_count_death_outcomes data contains all years
#* only need to load this once while loop goes through all years
zip_count_death_outcomes <- read_rds(paste0(data_path, "/data_process_mortality/mortality_dta/dod_after_first_adrd_hosp_zip.rds")) %>% 
  rename(zip = zip_dod,
         CaseDateTime = bene_dod) 
...
gen_data4casecross_year <- function(year){
  data_w_lagged_wfpm <- read_fst(paste0(data_path_data4casecross, "/data_w_lagged_wfpm/data4casecross_", year, ".fst")) %>% 
    select(dayName, CaseDateTime, DayDateTime, id, starts_with(c("zip", "wfpm"))) #set the dayName (Case vs ControlXX) as the id bc we work with aggregated outcome
  ...
  death_dta <- left_join(data_w_lagged_wfpm, data_w_lagged_temp_rh, by = c("zip", "DayDateTime")) %>% 
    left_join(., zip_count_death_outcomes, by = c("zip",
...
for (year in 1:length(years)){
  ...
  data4casecross_zip[[year]] <- data_year_no_missing_conf_exp
}

# there are 17 626 048 case AND control zip-day hospitalizations of interest (overlap possible, e.g., a zip day could have cardioresp and adrd)
data4casecross_zip_final <- bind_rows(data4casecross_zip) %>% 
  arrange(id) %>% 
  select(-CaseDateTime) %>% 
  left_join(., strat_pov_urbanicity, by = "zip") %>% 
  mutate(pct_pov_final = ifelse(year(DayDateTime) <= 2011, pct_pov_0711, pct_pov_1216),
         med_hh_inc_final = ifelse(year(DayDateTime) <= 2011, med_hh_inc_0711, med_hh_inc_1216)) %>% # keep only hospitalizations of interest bc a person could be hospitalized for many other reasons
  select(-contains(c("0711", "1216")))

data4casecross_zip_final_w_wfpm_cluster <- data4casecross_zip_final %>% 
  left_join(., clusters_wfpm_5_w_cluster_name %>% 
              rename(DayDateTime = date)) #note that the NA clusters are because they exist beyond 4-10 months

write_fst(data4casecross_zip_final_w_wfpm_cluster, '/n/dominici_nsaph_l3/Lab/projects/wfpm_adrd_hosp/data/data_process_mortality/data4casecross/data_w_all_vars/data4casecross_zip.fst', compress = 100)
